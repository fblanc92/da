...
class ConfigUtilTest(tf.test.TestCase):
  ...
  def test_get_configs_from_pipeline_file(self):
    """Test that proto config can be read from pipeline config file."""
    pipeline_config_path = os.path.join(self.get_temp_dir(), "pipeline.config")
  ...
  def test_create_configs_from_pipeline_proto(self):
    """Tests creating config dictionary from pipeline proto."""
  ...
  def test_create_pipeline_proto_from_configs(self):
    """Tests that proto can be reconstructed from config dictionary."""
    pipeline_config_path = os.path.join(self.get_temp_dir(), "pipeline.config")
  ...
  def test_get_configs_from_multiple_files(self):
    """Tests that proto config can be read from multiple files."""
  ...
  def testGenericConfigOverride(self):
    """Tests generic config overrides for all top-level config."""
    # Set one parameter for each of the top-level pipeline config:
    pipeline_config = pipeline_pb2.TrainEvalPipelineConfig()
  ...
  def testCheckAndParseInputConfigKey(self):
    ...
    with self.assertRaisesRegexp(ValueError,
                                 "Invalid key format when overriding config."):
      config_util.check_and_parse_input_config_key(
