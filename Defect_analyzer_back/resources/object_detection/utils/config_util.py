...
def create_configs_from_pipeline_proto(pipeline_config):
  """Creates a config dictionary from pipeline_pb2.TrainEvalPipelineConfig.
...
def create_pipeline_proto_from_configs(configs):
  """Creates a pipeline_pb2.TrainEvalPipelineConfig from config dictionary.

  This function performs the inverse operation of
  create_configs_from_pipeline_proto().

  Args:
    configs: Dictionary of config. See get_configs_from_pipeline_file().
...
def check_and_parse_input_config_key(configs, key):
  ...
  if len(fields) == 1:
    ...
    raise ValueError("Invalid key format when overriding config.")
  ...
  # should match config[key_name].name, for eval input config it should match
...
def merge_external_params_with_configs(configs, hparams=None, kwargs_dict=None):
  """Updates `config` dictionary based on supplied parameters.

  This utility is for modifying specific fields in the object detection config.
  ...
  In this case key can be one of the following formats:
      ...
      2. specific update: colon separated string that indicates which field in
        ...
        - input_name: a 'name' that can be used to identify elements, especially
          when config[key_name] is a repeated field.
        - field_name: name of the field that you want to override.
        For example, given config dict as below:
          config = {
            'model': {...}
            'train_config': {...}
            'train_input_config': {...}
...
def _maybe_update_config_with_key_value(configs, key, value):
  """Checks key type and updates `config` with the key value pair accordingly.
...
def _update_initial_learning_rate(configs, learning_rate):
  """Updates `config` to reflect the new initial learning rate.
  ...
  The config dictionary is updated in place, and hence not returned.
...
def _update_batch_size(configs, batch_size):
  """Updates `config` to reflect the new training batch size.
...
def _update_momentum_optimizer_value(configs, momentum):
  """Updates `config` to reflect the new momentum value.

  Momentum is only supported for RMSPropOptimizer and MomentumOptimizer. For any
  other optimizer, no changes take place. The config dictionary is updated in
...
def _update_classification_localization_weight_ratio(configs, ratio):
  ...
  Detection classes usually define a loss weight for both classification and
...
def _update_train_steps(configs, train_steps):
  """Updates `config` to reflect new number of training steps."""
  configs["train_config"].num_steps = int(train_steps)


def _update_all_eval_input_configs(configs, field, value):
  """Updates the content of `field` with `value` for all eval input config."""
...
def _update_use_bfloat16(configs, use_bfloat16):
  """Updates `config` to reflect the new setup on whether to use bfloat16.
